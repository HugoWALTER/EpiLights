	...
	return {
		setLight: (planning, led) => {
			...
			Array.from(new Set(planning.map((v) => v.room))).forEach((roomName) => {
				let inThisRoomCourses = planning.filter((elem) => elem.room == roomName)
				let occupedRoomCourses = inThisRoomCourses.filter((elem) => {
					let start = new Date(elem.start)
					let end = new Date(elem.end)
					return time >= start && time < end
				})
				let soonOccupedRoomCourses = inThisRoomCourses.filter((elem) => {
					let start = new Date(elem.start)

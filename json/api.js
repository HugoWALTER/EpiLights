...
module.exports = () => {
	return {
		getUser: (token) => {
			return axios.get(`${API}/${token}/user/?format=json`).then((res) => res.data)
		},
		getPlanning: (token, user) => {
			const d = new Date()
			const dString = `${d.getFullYear()}-${d.getMonth() + 1}-${d.getDate()}`
			return axios
				.get(`${API}/${token}/planning/load?format=json&start=${dString}&end=${dString}`)
				.then((res) =>
					(Array.isArray(res.data) ? res.data : []).filter(
						(f) => f.instance_location === user.location,
					),
				)
		},
		getRoom: (planning) => {
